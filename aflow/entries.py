...
class AflowFile(object):
    ...
    def __call__(self, target=None):
        ...
        # instead.
        # Do not replace colon to slash unless after the edu domain.
        url = "http://{}/{}".format(self.aurl.replace("edu:", "edu/"), self.filename)
        try:
...
class AflowFiles(list):
    ...
    def __init__(self, entry):
        ...
        # The aurl here has replaced "edu:" to "edu/"
...
#     _loaders[keyword] = lambda x: x._lazy_load(str(keyword))
...
class Entry(object):
    ...
    def _lazy_load(self, keyword):
        ...
        Args:
            ...
            update _lazy_load method so that it uses the `aflowlib.json` parser
        """
        if keyword in self.attributes:
            ...
            for k, v in raw_responses.items():
                if k not in self.attributes.keys():
                    ...
                    self.attributes[k] = res
            # Still return values
            # import pdb; pdb.set_trace()
            try:
                result = self.attributes[keyword]
